...
export const name =
  "arrange() can read from a well formed JSON file by default";
...
export const before = (async (context) => {
  ...
  const randomFile = resolve(tmpdir(), [randomString(), ".", "json"].join(""));
  const jsonString = stringify(REFERENCE_OBJECT);
  await promises.writeFile(randomFile, jsonString);
